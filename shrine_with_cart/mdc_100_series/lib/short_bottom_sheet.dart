      ...
      _heightAnimation = TweenSequence(
        <TweenSequenceItem<double>>[
          TweenSequenceItem<double>(
            tween: Tween<double>(
                    begin: _cartHeight,
                    end: _cartHeight + (mediaHeight - _cartHeight) * _peakVelocityProgress)
                .chain(CurveTween(curve: _accelerateCurve)),
            weight: _peakVelocityTime,
          ),
          TweenSequenceItem<double>(
            tween: Tween<double>(
                    begin: _cartHeight + (mediaHeight - _cartHeight) * _peakVelocityProgress,
                    end: mediaHeight)
                .chain(CurveTween(curve: _decelerateCurve)),
            weight: 1 - _peakVelocityTime,
          ),
...
*/
      ...
      const Cubic fastInSlowOut = Cubic(0.2, 0.0, 0.4, 1.0);
      _heightAnimation =
          Tween<double>(begin: _cartHeight, end: mediaHeight).animate(
        CurvedAnimation(
          curve: Interval(
            0.2,
            1.0,
...
/*      _heightAnimation = TweenSequence(
      ...
      _shapeAnimation = TweenSequence(
        <TweenSequenceItem<double>>[
          ...
          TweenSequenceItem<double>(
            tween: Tween<double>(
                    begin: cornerRadius * _peakVelocityProgress, end: 0.0)
                .chain(CurveTween(curve: _decelerateCurve)),
            weight: _peakVelocityTime,
          ),
        ],
    ...
    _thumbnailOpacityAnimation = Tween<double>(begin: 1.0, end: 0.0).animate(
      CurvedAnimation(
          ...
          curve: _controller.status == AnimationStatus.forward
              ...
              : Interval(0.53, 0.77, curve: Curves.linear)),
    );
    ...
    _cartOpacityAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
          ...
          curve: _controller.status == AnimationStatus.forward
              ...
              : Interval(0.77, 1.0, curve: Curves.linear)),
    );
  }
